...
const PROJECT_ID = 'enterprise-rag-knowledge-ai'
const SITE_URL = 'https://aicmode-portfolio.vercel.app'
const PATH = '/works/enterprise-rag-knowledge-ai'

const project = projects.find((entry) => entry.id === PROJECT_ID)

const title = 'Enterprise RAG Knowledge AI｜社内PDFを根拠付きで検索するAI — AIC'
const description =
  '社内PDFをテキスト抽出・OCR・チャンク化し、OpenAI EmbeddingsとPostgreSQL / pgvectorでベクトル検索。資料名・ページ・引用箇所付きのRAG回答を生成する社内ナレッジ検索AIです。'
...
export default function EnterpriseRagKnowledgeAiPage() {
  if (!project) notFound()
          ...
          <AnimateIn delay={80}>
            <div className="mt-20 flex flex-col gap-6 border-t border-white/[0.08] pt-8 md:mt-28 lg:flex-row lg:items-center lg:justify-between">
              <p className="max-w-xl text-[11.5px] leading-6 tracking-[0.04em] text-white/55">
                自分で企画・制作した社内ナレッジ検索AIです。公開しているのは、登録不要でPDF解析から根拠付き回答まで試せる匿名デモ環境です。
              </p>
